...
public class ProducerRouteContext extends RouteContext
{
   ...
   {
      return Util.getRouteId(partitionId, application.getModel().getId(), null,
   ...
   public boolean addApplicationAttributesToHeaders()
   {
      ...
      if (isProducerApplication(application)
            && !StringUtils.isEmpty(providedRoute))
      {
         if (providedRoute.contains("://service/"))
            return true;
      }

      Object value = application.getAttribute("carnot:engine:camel::includeAttributesAsHeaders");
