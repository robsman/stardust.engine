...
import static org.eclipse.stardust.engine.extensions.camel.CamelConstants.SQL_OVERLAY;
...
public class ProducerRouteContextFactory
{
         ...
         CamelContext context, String partition)
   {
      if(Util.getOverlayType(application)!=null){
         ...
         if(Util.getOverlayType(application).equalsIgnoreCase(SQL_OVERLAY))
            return new SqlApplicationRouteContext(application, partition, context.getName());
      }
