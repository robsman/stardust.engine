...
public class CamelDmsUtils
{
   ...
   public static Document storeDocument(DocumentManagementService dms, ProcessInstance pi,
         byte[] content, String fileName, Boolean processAttachmentSupport)
               throws CreateDocumentException
   {

      StringBuilder defaultPath = new StringBuilder(
            org.eclipse.stardust.engine.api.runtime.DmsUtils
                  .composeDefaultPath(pi.getScopeProcessInstanceOID(), pi.getStartTime()))
                        .append("/");
      ...
      Document doc = storeDocumentIntoDms(dms, defaultPath.toString(), content,
            sanitize(fileName));
         ...
         String folderId, byte[] content, String fileName) throws CreateDocumentException
   {
      ...
      try
      {
         DocumentInfo docInfo = org.eclipse.stardust.engine.api.runtime.DmsUtils
         ...
         if (doc == null)
         {
            doc = dms.createDocument(folderId, docInfo, extractContentByteArray(content),
                  null);
         }
   ...
   private static void ensureFolderExists(DocumentManagementService dms, String folderId)
         throws DocumentManagementServiceException
   {
      if (!StringUtils.isEmpty(folderId) && folderId.startsWith("/"))
      {
         ...
         {
            String parentFolderId = StringUtils.isEmpty(currentPath) ? "/" : currentPath;
   ...
   public static Document toDocument(Object messageContent, Exchange exchange,
         String fileName, DocumentManagementService dms, ProcessInstance pi,
         boolean processAttachmentSupport)
   {
      ...
      try
      {
         ...
         document = CamelDmsUtils.storeDocument(dms, pi, jcrDocumentContent, fileName,
   ...
   public static List<String> getDocumentAccessPointListForTrigger(
         ProcessDefinition processDefinition)
   {
      ...
      while (triggers.hasNext())
      {
         ...
         Iterator<ParameterMapping> ParameterMappings = trigger.getAllParameterMappings()
               .iterator();
         while (ParameterMappings.hasNext())
         {
            ...
            if (accessPoint.getAccessPathEvaluatorClass()
                  .equals(CamelConstants.VFS_DOCUMENT_ACCESS_PATHE_EVALUATOR_CLASS))
            {
         ...
         throws IOException, MessagingException
   {
      ...
      if (exchange != null)
      {
         ...
         else if (messageContent instanceof MimeMultipart)
         {
            ...
            jcrDocumentContent = MimeMultipartTypeConverter
                  .mimeMultipartToString(mimeMultipart).getBytes();
         }
         else if (messageContent instanceof InputStream)
         {
            jcrDocumentContent = IOUtils
                  .toByteArray(InputStream.class.cast(messageContent));
         }
   ...
   public static Map<String, Document> initializeDocumentData(Trigger trigger,
         ProcessInstance pi, Exchange exchange, ServiceFactory sf)
   {
      ...
      while (parameterMappings.hasNext())
      {
         ...
         CamelMessageLocation location = accessPoint.getId().equals(bodyOutAP)
               || bodyOutAP == null
               ...
               .equals(CamelConstants.VFS_DOCUMENT_ACCESS_PATHE_EVALUATOR_CLASS))
         {
            ...
            if (CamelMessageLocation.HEADER.equals(location))
            {
               ...
               {
                  initialDocumentDataValues.put(parameterMapping.getDataId(),
                        toDocument(messageContent, exchange, accessPoint.getName(),
                              sf.getDocumentManagementService(), pi, false));
               }
   ...
   private static String getDocumentName(Object messageContent, String fileName)
   {
      if (FilenameUtils.getExtension(fileName).isEmpty())
      {
         ...
         {
            String ext = MimeTypeUtils
                  .getExtensionFromBodyPartContentType((MimeMultipart) messageContent);
            if (!ext.isEmpty())
            {
               fileName += "." + ext;
            }
   ...
   public static Document getDocumentUsingRepositoryLocation(
         DocumentManagementService dms, String repositoryLocation)
   {
      Document document = null;
      if (!isRepositoryDocumentId(repositoryLocation))
      {
   ...
   private static boolean isRepositoryDocumentId(String id)
   {

      if (id.startsWith(REPOSITORY_ID_PREFIX))
         return true;
      return false;
   }
}
