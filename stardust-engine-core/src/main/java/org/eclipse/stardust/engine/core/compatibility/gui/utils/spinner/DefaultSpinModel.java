      ...
      implements SpinModel, ChangeListener
{
   ...
   public DefaultSpinModel(double iValue, double iExtent, double iMin, double iMax, boolean iWrap,
         double fValue, double fExtent, double fMin, double fMax, boolean fWrap)
   {
      SpinRangeModel iModel = new DefaultSpinRangeModel(iValue, iExtent, iMin, iMax,
            iWrap);
      setRange(NumberFormat.INTEGER_FIELD, iModel);

      SpinRangeModel fModel = new DefaultSpinRangeModel(fValue, fExtent, fMin, fMax,
            fWrap);
