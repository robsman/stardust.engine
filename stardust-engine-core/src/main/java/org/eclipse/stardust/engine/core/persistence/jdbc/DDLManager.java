...
public class DDLManager
{
      ...
      try
      {
         if ( !containsTable(schemaName, dataCluster.getTableName(), connection))
         {
            ...
            try
            {
               ...
               for (AbstractDataClusterSlot slot : dataCluster.getAllSlots())
               {
   ...
   private static String getDataValueField(ClusterSlotFieldInfo dataSlotField)
   {
   ...
   public static String getColumnValuesSelect(Collection<ClusterSlotFieldInfo> columns, final String dataValuePrefix, String dataValueSelect)
   {
      ...
      Iterator<ClusterSlotFieldInfo> i = columns.iterator();
      while(i.hasNext())
      {
         ClusterSlotFieldInfo tmp = i.next();
   ...
   public static String getColumnValueSelect(ClusterSlotFieldInfo fieldInfo, String dataValuePrefix, String dataValueSelect)
   {
      StringBuilder builder = new StringBuilder();
         ...
         String schemaName, PrintStream spoolFile, String statementDelimiter, PrintStream consoleLog)
   {
      ...
      {
         int countInconsistencyGroups = 0;
         ...
         try
         {
            ...
            for (AbstractDataClusterSlot clusterSlot : syncInfo.getDataSlots(dataCluster))
            {
               if (syncInfo.getPerformClusterVerification())
               {
                  Statement verifyStmt = null;
                  try
                  {
                     ...
                     if (existsReferencedDVsByDCNotMatching(verifyStmt, clusterSlot,
               ...
               // any inconsistencies counted? Then prepare and execute synch statements
               if (countInconsistencyGroups != 0)
               {
                  ...
                  {
                     Map<SLOT_TYPE, ClusterSlotFieldInfo> dataSlotColumnsToSynch = syncInfo
                           .getDataSlotColumns(clusterSlot);
                     if(dataSlotColumnsToSynch.size() != 0)
                     {
                        ...
                        if (dbDescriptor.supportsMultiColumnUpdates())
                        {
                           ...
                           StringUtils.join(buffer, iter, ", ");
                           buffer.append(")");
                           buffer.append(" = ");
                           buffer.append("(");
                           buffer.append(
                                 getColumnValuesSelect(dataSlotColumnsToSynch.values(),
                        ...
                        if (syncInfo.getScopePiOid() != 0 || mixedSlots)
                        {
                           ...
                           if (mixedSlots)
                           {
                              ...
                              buffer.append(MessageFormat.format("({0} IS NULL OR {1} = {2})",
                                    new Object[] {
                                          oidField.getName(), // 0
                                          typeField.getName(), // 1,
                                          BigData.NULL // 2
         ...
         if (countInconsistencyGroups != 0)
         {
            ...
            message.append(countInconsistencyGroups == 1
                  ? ". There was 1 inconsistency. "
                  : ". There were " + countInconsistencyGroups + " inconsistencies. ");
         ...
         DataCluster dataCluster, String schemaName)
   {
      ...
      // data cluster tab
      result = StringUtils.replace(result, "_$DC_TAB$_", clusterTable);
      ...
      result = StringUtils.replace(result, "_$DV_PROCINSTANCE$_", DataValueBean.FIELD__PROCESS_INSTANCE);
      result = StringUtils.replace(result, "_$DV_TYPE_KEY$_", DataValueBean.FIELD__TYPE_KEY);
      result = StringUtils.replace(result, "_$DV_PROCESSINSTANCE$_", DataValueBean.FIELD__PROCESS_INSTANCE);
      ...
      result = StringUtils.replace(result, "_$SD_MODEL$_", StructuredDataBean.FIELD__MODEL);
      result = StringUtils.replace(result, "_$SD_DATA$_", StructuredDataBean.FIELD__DATA);
   ...
   private String replaceSqlFragmentsBySlot(String sqlTemplate, AbstractDataClusterSlot dataSlot)
   {
      String result;

      result = StringUtils.replace(sqlTemplate, "_$SLOT_COL_OID$_", dataSlot.getOidColumn());
      result = StringUtils.replace(result, "_$SLOT_COL_TYPE_KEY$_", dataSlot.getTypeColumn());
//      result = StringUtils.replace(result, "_$SLOT_DATA_ATTRIBUTE$_", dataSlot.getAttributeName());
//      result = StringUtils.replace(result, "_$SLOT_DATA_MODEL_ID$_", dataSlot.getModelId());
      ...
      if ( !StringUtils.isEmpty(dataSlot.getSValueColumn()))
      {
         ...
         result = StringUtils.replace(result, "_$SLOT_COL_VALUE$_", dataSlot.getSValueColumn());
      }
      else
      {
         result = StringUtils.replace(result, "_$DV_VALUE$_",
               DataValueBean.FIELD__NUMBER_VALUE);
         result = StringUtils.replace(result, "_$SLOT_COL_VALUE$_", dataSlot.getNValueColumn());
      }
...
//      result = StringUtils.replace(result, "_$SLOT_DATA_ID$_", dataSlot.getDataId());
   ...
   private boolean existsReferencedDVsByDCNotMatching(Statement verifyStmt,
         AbstractDataClusterSlot clusterSlot, DataCluster dataCluster, String schemaName)
         throws SQLException
   {
      ...
      if (clusterSlot.hasPrimitiveData())
      {
         ...
         if (!primitiveDataRtOids.isEmpty())
         {
            ...
            {
               modelOids.add((long) data.getModel().getModelOID());
            }

            String syncStringTemplate =
                  "SELECT 'x' FROM _$DC_TAB$_ dc, _$PI_TAB$_ pi " +
                  "WHERE " +
                     "dc._$DC_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                     "dc._$SLOT_COL_OID$_ IS NOT NULL AND " +
                     "pi._$PI_MODEL$_ IN (" + StringUtils.join(modelOids.iterator(), ",") + ") AND " +
                     "NOT EXISTS( " +
                        "SELECT 'x' " +
                        "FROM _$DV_TAB$_ dv " +
                        "WHERE " +
                           "dv._$DV_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                           "dv._$DV_DATA$_ IN (" + StringUtils.join(primitiveDataRtOids.keySet().iterator(), ",") + ") AND " +
                           "dv._$DV_OID$_ = dc._$SLOT_COL_OID$_ AND " +
                           "dv._$DV_TYPE_KEY$_ = dc._$SLOT_COL_TYPE_KEY$_ AND " +
                           "(dv._$DV_VALUE$_ = dc._$SLOT_COL_VALUE$_ OR " +
                              "(dv._$DV_VALUE$_ IS NULL AND dc._$SLOT_COL_VALUE$_ IS NULL)))";

            syncStringTemplate = replaceSqlFragmentsByCluster(syncStringTemplate, dataCluster,
                  schemaName);
            String syncStmtString = replaceSqlFragmentsBySlot(syncStringTemplate, clusterSlot);

            verifyStmt.execute(syncStmtString);
            defectReferenceExists = verifyStmt.getResultSet().next();
         }
      }

      // if defect was found for primitive data it still might be correct for structured data
      if (defectReferenceExists && clusterSlot.hasStructuredData())
      {
         Map<Long, Pair<IData, String>> structuredDataRtOids = Collections.EMPTY_MAP;
         structuredDataRtOids = DataClusterHelper
               .findAllStructuredDataRtOids(clusterSlot);
         if (!structuredDataRtOids.isEmpty())
         {
            anyReferencedDataDeployed = true;
            ...
            {
               modelOids.add((long) data.getFirst().getModel().getModelOID());
            }

            String syncStringTemplateStruct =
                  "SELECT 'x' FROM _$DC_TAB$_ dc, _$PI_TAB$_ pi, _$MODEL_TAB$_ m " +
                  "WHERE " +
                     "dc._$DC_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                     "dc._$SLOT_COL_OID$_ IS NOT NULL AND " +
                     "pi._$PI_MODEL$_ IN (" + StringUtils.join(modelOids.iterator(), ",") + ") AND " +
                     "NOT EXISTS( " +
                        "SELECT 'x' " +
                        "FROM _$SDV_TAB$_ sdv, _$SD_TAB$_ sd, _$DV_TAB$_ dv " +
                        "WHERE " +
                           "sdv._$SDV_PROCESSINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                           "sdv._$SDV_XPATH$_ IN (" + StringUtils.join(structuredDataRtOids.keySet().iterator(), ",") + ") AND " +
                           ...
                           "dv._$DV_OID$_ = dc._$SLOT_COL_OID$_ AND " +
                           "sdv._$DV_TYPE_KEY$_ = dc._$SLOT_COL_TYPE_KEY$_ AND " +
                           "(sdv._$DV_VALUE$_ = dc._$SLOT_COL_VALUE$_ OR " +
                              "(sdv._$DV_VALUE$_ IS NULL AND dc._$SLOT_COL_VALUE$_ IS NULL)))";

            syncStringTemplateStruct = replaceSqlFragmentsByCluster(syncStringTemplateStruct,
                  dataCluster, schemaName);
            String syncStmtString = replaceSqlFragmentsBySlot(syncStringTemplateStruct, clusterSlot);

            if (trace.isDebugEnabled())
            {
               trace.debug("About to execute: " + syncStmtString);
            }
    ...
    * @throws SQLException
    */
   private boolean existsEmptyDCEntryNotNull(Statement verifyStmt, AbstractDataClusterSlot clusterSlot,
         DataCluster dataCluster, String schemaName) throws SQLException
   {
      // get all models for this slot
      Set<Long> modelOids = CollectionUtils.newHashSet();
      if (clusterSlot.hasPrimitiveData())
      {
         Map<Long, IData> primitiveDataRtOids = DataClusterHelper
      ...
      boolean defectReferenceExists = false;
      if (!modelOids.isEmpty())
      {
         String syncStringTemplate =
               ...
               "WHERE " +
                  "dc._$DC_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                  "dc._$SLOT_COL_OID$_ IS NULL AND " +
                  "pi._$PI_MODEL$_ IN (" + StringUtils.join(modelOids.iterator(), ",") + ") AND " +
                  "(" +
                     "dc._$SLOT_COL_TYPE_KEY$_ IS NOT NULL OR " +
                     "dc._$SLOT_COL_VALUE$_ IS NOT NULL" +
                  ")";
      ...
      return defectReferenceExists;
   }

   /**
    * Check for slot entries in DC table where a specific slot does not reference any DV/SDV (slot columns are null)
    * but corresponding DV/SDV do exist.
    *
    * @param dataCluster
    * @param schemaName
    * @param verifyStmt
         ...
         Statement verifyStmt, AbstractDataClusterSlot clusterSlot) throws SQLException
   {
      ...
      if (clusterSlot.hasPrimitiveData())
      {
         ...
         if (!primitiveDataRtOids.isEmpty())
         {
            ...
            StringBuilder verifySqlTemplate = new StringBuilder(
                  "SELECT 'x' FROM _$DC_TAB$_ dc, _$PI_TAB$_ pi " +
                  "WHERE " +
                     "dc._$DC_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                     "dc._$SLOT_COL_OID$_ IS NULL AND " +
                     "dc._$SLOT_COL_TYPE_KEY$_ IS NULL AND " +
                     "dc._$SLOT_COL_VALUE$_ IS NULL AND " +
                     "pi._$PI_MODEL$_ IN (" + StringUtils.join(modelOids.iterator(), ",") + ") AND "
                     );

            verifySqlTemplate.append(
                  "EXISTS( " +
                     "SELECT 'x' " +
                     "FROM _$DV_TAB$_ dv " +
                     "WHERE " +
                        "dv._$DV_PROCINSTANCE$_ = pi._$PI_SCOPE_PI$_ AND " +
                        "dv._$DV_DATA$_ IN (" + StringUtils.join(primitiveDataRtOids.keySet().iterator(), ",") + ")" +
                        ")"
                        );

            String synchSQL = replaceSqlFragmentsByCluster(verifySqlTemplate.toString(), dataCluster,
                  schemaName);
            synchSQL = replaceSqlFragmentsBySlot(synchSQL, clusterSlot);

            verifyStmt.execute(synchSQL);
            defectReferenceExists =  verifyStmt.getResultSet().next();
         }
      }
   ...
   public static class SubSelectDescriptor
   {
      ...
      public SubSelectDescriptor(String subSelectSql, String dataValuePrefix)
      {
         this.subSelectSql = subSelectSql;
         this.dataValuePrefix = dataValuePrefix;
      }
