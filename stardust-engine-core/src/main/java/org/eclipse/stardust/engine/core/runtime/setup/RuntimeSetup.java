               ...
               if (0 < atNodes.getLength())
               {
                  ...
                  if (0 < dccNodes.getLength())
                  {
                     ...
                     for (int dcIdx = 0; dcIdx < dcNodes.getLength(); ++dcIdx)
                     {
                        ...
                        NodeList dscNodes = dcNode
                              .getElementsByTagName(DATA_SLOTS);
                        if (0 < dscNodes.getLength())
                        {
                           if (1 < dscNodes.getLength())
                           {
                              trace.warn("Ignoring all but first <" + DATA_SLOTS + "> elements");
                           }
                           ...
                           for (int dsIdx = 0; dsIdx < dsNodes.getLength(); ++dsIdx)
                           {
                              ...
                              String attribute = dsNode.getAttribute(DATA_SLOT_ATTRIBUTENAME_ATT);
                              ...
                              else
                              {
                                 trace.warn("Will ignore data slot for cluster table: "
                                       ...
                                       + modelId + ", dataId: " + dataId
                                       + ", attribute: " + attribute);
                              }
                           }
                        }
                        
                        NodeList dcicNodes = dcNode.getElementsByTagName(DATA_CLUSTER_INDEXES);
                        if (0 < dcicNodes.getLength())
                        {
                           if (1 < dcicNodes.getLength())
                           {
                              trace.warn("Ignoring all but first <" + DATA_CLUSTER_INDEXES + "> elements");
                           }
                           ...
                           for (int dciIdx = 0; dciIdx < dciNodes.getLength(); ++dciIdx)
                           {
                              ...
                              List columnNames = new ArrayList(cnNodes.getLength());
                              ...
                              indexes.add(new DataClusterIndex(
                                    dcTableName,
                                    dciNode.getAttribute(INDEX_NAME_ATT),
                                    StringUtils.getBoolean(dciNode.getAttribute(INDEX_UNIQUE_ATT), false),
                                    columnNames
                                    ));
                           }
                        }
                        ...
                        clusters.add(new DataCluster(
                              schemaName,
                              dcTableName,
                              dcNode.getAttribute(DATA_CLUSTER_PICOLUMN_ATT),
                              (DataSlot[]) slots.toArray(new DataSlot[0]),
                              (DataClusterIndex[]) indexes.toArray(new DataClusterIndex[0])));
                     }
                  }
               }
            }
         }
         this.clusters = (DataCluster[]) clusters.toArray(EMPTY_DATA_CLUSTER_ARRAY);
      }
