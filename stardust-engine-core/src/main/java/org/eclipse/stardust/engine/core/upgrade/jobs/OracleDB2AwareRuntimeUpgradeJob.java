...
public abstract class OracleDB2AwareRuntimeUpgradeJob extends RuntimeUpgradeJob
{
   protected void assertCompatibility() throws UpgradeException
   {
      if (!(item.getDbDescriptor().getDbmsKey().equals(DBMSKey.ORACLE)
            ...
            || item.getDbDescriptor().getDbmsKey().equals(DBMSKey.DB2_UDB)))
      {
         throw new UpgradeException("The upgrade job for version " + getVersion()
               + " is only valid for " + DBMSKey.ORACLE.getName() + ", "
               + DBMSKey.ORACLE9i + " or " + DBMSKey.DB2_UDB.getName()
               + " databases and thus will not work with the current "
