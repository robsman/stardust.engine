...
<xsl:stylesheet version="1.0" exclude-result-prefixes="wfm xpdl"
   xmlns:x1="http://www.wfmc.org/2002/XPDL1.0"
   xmlns:xpdl="http://www.wfmc.org/2008/XPDL2.1"
   xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   xmlns:wfm="http://www.carnot.ag/workflowmodel/3.1">
      ...
      <Activity xmlns="http://www.wfmc.org/2008/XPDL2.1">
         ...
         <xsl:choose>
            ...
            <xsl:otherwise>
               ...
               <xsl:if test="(@loopType = 'While') or (@loopType = 'Repeat')">
                  <Loop LoopType="Standard">
                     <LoopStandard>
                        <xsl:attribute name="TestTime">
                           <xsl:choose>
                              <xsl:when test="@loopType = 'While'">Before</xsl:when>
                              <xsl:otherwise>After</xsl:otherwise>
                           </xsl:choose>
                        </xsl:attribute>
                        <xsl:if test="@loopCondition">
                           <LoopCondition><xsl:value-of select="@loopCondition"/></LoopCondition>
                        </xsl:if>
                     </LoopStandard>
                  </Loop>
               </xsl:if>
      ...
      <DataType xmlns="http://www.wfmc.org/2008/XPDL2.1">
         <xsl:choose>
            ...
            <xsl:when test="@type='struct'">
               <xsl:choose>
                  <xsl:when test="x1:ExternalReference|xpdl:ExternalReference">
                     <xsl:copy-of select="x1:ExternalReference|xpdl:ExternalReference"/>
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:variable name="dataType" select="wfm:attribute[@name='carnot:engine:dataType']/@value"/>
                     <xsl:choose>
