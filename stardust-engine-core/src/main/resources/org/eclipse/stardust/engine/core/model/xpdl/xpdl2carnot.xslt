...
<xsl:stylesheet version="1.0" exclude-result-prefixes="x1 xpdl c"
   ...
   <xsl:template match="/">
      <xsl:apply-templates select="x1:Package|xpdl:Package"/>
   </xsl:template>

   <xsl:template match="x1:Package|xpdl:Package">
      <model>
         <!-- attributes -->
            <xsl:call-template name="xpdl-id-and-name"/>

         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
         ...
         <xsl:variable name="PackageHeader" select="x1:PackageHeader|xpdl:PackageHeader"/>
         <xsl:for-each select="$PackageHeader">
            <xsl:variable name="Vendor" select="x1:Vendor|xpdl:Vendor"/>
            <xsl:if test="$Vendor">
               <xsl:attribute name="vendor"><xsl:value-of select="$Vendor"/></xsl:attribute>
            </xsl:if>
            <xsl:variable name="Created" select="x1:Created|xpdl:Created"/>
            <xsl:if test="$Created">
               <xsl:attribute name="created"><xsl:value-of select="$Created"/></xsl:attribute>
            </xsl:if>
         </xsl:for-each>

         <xsl:variable name="RedefinableHeader" select="x1:RedefinableHeader|xpdl:RedefinableHeader"/>
         <xsl:for-each select="$RedefinableHeader">
            <xsl:variable name="Author" select="x1:Author|xpdl:Author"/>
            <xsl:if test="$Author">
         ...
         <!-- child elements -->
         <xsl:for-each select="$RedefinableHeader">
            <xsl:variable name="Version" select="x1:Version|xpdl:Version"/>
            <xsl:if test="$Version">
               <attribute>
         ...
         <xsl:for-each select="x1:DataFields|xpdl:DataFields">
               <xsl:apply-templates select="x1:DataField|xpdl:DataField"/>
         </xsl:for-each>

         <xsl:for-each select="x1:Applications|xpdl:Applications">
               <xsl:apply-templates select="x1:Application|xpdl:Application"/>
         </xsl:for-each>

            <xsl:apply-templates select="x1:Participants|xpdl:Participants"/>

         <xsl:for-each select="$Package">
               <xsl:apply-templates select="c:Modelers/c:Modeler"/>
         </xsl:for-each>

         <xsl:for-each select="x1:WorkflowProcesses|xpdl:WorkflowProcesses">
               <xsl:apply-templates select="x1:WorkflowProcess|xpdl:WorkflowProcess"/>
         </xsl:for-each>
   ...
   <xsl:template match="x1:Participants|xpdl:Participants">
      <xsl:for-each select="x1:Participant|xpdl:Participant">
         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
         <xsl:variable name="CarnotExt" select="$attrs[@Name='CarnotExt']"/>
         <xsl:variable name="ParticipantType" select="x1:ParticipantType|xpdl:ParticipantType"/>
         <xsl:variable name="ExternalReferenceType" select="x1:ExternalReference|xpdl:ExternalReference"/>
         <xsl:choose>
   ...
   <xsl:template name="carnot-role">
      ...
      <role>
         ...
         <xsl:for-each select="$Role">
            ...
            <xsl:call-template name="xpdl-id-and-name"/>
         <xsl:if test="x1:ExternalReference|xpdl:ExternalReference">
            <xsl:call-template name="carnot-element-proxy">
               <xsl:with-param name="ref" select="x1:ExternalReference|xpdl:ExternalReference"/>
            </xsl:call-template>
         </xsl:if>
   ...
   <xsl:template match="x1:Application|xpdl:Application">
      <application>
         <!-- attributes -->
         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
         <xsl:variable name="CarnotExt" select="$attrs[@Name='CarnotExt']"/>
   ...
   <xsl:template match="x1:DataField|xpdl:DataField">
      <data>
         <!-- attributes -->
         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
            <xsl:variable name="CarnotExt" select="$attrs[@Name='CarnotExt']"/>
         ...
         <xsl:variable name="DataType" select="x1:DataType|xpdl:DataType"/>
         <xsl:variable name="BasicType" select="$DataType/x1:BasicType|$DataType/xpdl:BasicType"/>
         <xsl:variable name="DeclaredType" select="$DataType/x1:DeclaredType|$DataType/xpdl:DeclaredType"/>
         <xsl:variable name="SchemaType" select="$DataType/x1:SchemaType|$DataType/xpdl:SchemaType"/>
         <xsl:variable name="ExternalReference" select="$DataType/x1:ExternalReference|$DataType/xpdl:ExternalReference"/>
         ...
         <xsl:variable name="filter2">
            ...
            <xsl:for-each select="x1:Length|xpdl:Length">
                <xsl:message>Ignoring array length for data <xsl:value-of select="../@Id"/>.</xsl:message>
            </xsl:for-each>
      </data>
   </xsl:template>

   <xsl:template match="x1:WorkflowProcess|xpdl:WorkflowProcess">
      <processDefinition>
         <!-- attributes -->
         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
            <xsl:variable name="CarnotExt" select="$attrs[@Name='CarnotExt']"/>
         ...
         <xsl:variable name="ProcessHeader" select="x1:ProcessHeader|xpdl:ProcessHeader"/>
         <xsl:variable name="Priority" select="$ProcessHeader/x1:Priority|$ProcessHeader/xpdl:Priority"/>
         ...
         <xsl:variable name="DataFields" select="x1:DataFields|xpdl:DataFields"/>
            <xsl:for-each select="$DataFields/x1:DataField|$DataFields/xpdl:DataField">
                <xsl:message>Ignoring process-local data <xsl:value-of select="@Id"/> for process <xsl:value-of select="../../@Id"/></xsl:message>
            </xsl:for-each>

         <xsl:variable name="Participants" select="x1:Participants|xpdl:Participants"/>
            <xsl:for-each select="$Participants/x1:Participant|$Participants/xpdl:Participant">
                <xsl:message>Ignoring process-local participant <xsl:value-of select="@Id"/> for process <xsl:value-of select="../../@Id"/></xsl:message>
            </xsl:for-each>

         <xsl:variable name="Applications" select="x1:Applications|xpdl:Applications"/>
            <xsl:for-each select="$Applications/x1:Application|$Applications/xpdl:Application">
                <xsl:message>Ignoring process-local application <xsl:value-of select="@Id"/> for process <xsl:value-of select="../../@Id"/></xsl:message>
            </xsl:for-each>

         <xsl:variable name="ActivitySets" select="x1:ActivitySets|xpdl:ActivitySets"/>
            <xsl:for-each select="$ActivitySets/x1:ActivitySet|$ActivitySets/xpdl:ActivitySet">
                <xsl:message>Ignoring activity set <xsl:value-of select="@Id"/> for process <xsl:value-of select="../../@Id"/></xsl:message>
            </xsl:for-each>

         <xsl:variable name="Activities" select="x1:Activities|xpdl:Activities"/>
            <xsl:apply-templates select="$Activities/x1:Activity|$Activities/xpdl:Activity"/>

         <xsl:variable name="Transitions" select="x1:Transitions|xpdl:Transitions"/>
            <xsl:apply-templates select="$Transitions/x1:Transition|$Transitions/xpdl:Transition"/>
            ...
            <xsl:copy-of select="x1:FormalParameters|xpdl:FormalParameters"/>
   ...
   <xsl:template match="x1:Activity|xpdl:Activity">
      <xsl:element name="activity">
         ...
         <xsl:variable name="Activity" select="$CarnotExt/c:Activity"/>
            ...
            <!-- TODO @application -->
         <xsl:variable name="Implementation" select="x1:Implementation|xpdl:Implementation"/>
         <xsl:variable name="No" select="$Implementation/x1:No|$Implementation/xpdl:No"/>
         <xsl:variable name="Tool" select="$Implementation/x1:Tool|$Implementation/xpdl:Tool"/>
         <xsl:variable name="TaskApplication" select="$Implementation/xpdl:Task/xpdl:TaskApplication"/>
         <xsl:variable name="SubFlow" select="$Implementation/x1:SubFlow|$Implementation/xpdl:SubFlow"/>
         ...
         <xsl:variable name="BlockActivity" select="x1:BlockActivity|xpdl:BlockActivity"/>
         ...
         <xsl:if test="$SubFlow">
            <xsl:if test="not($SubFlow/@PackageRef)">
                ...
                <!-- TODO @Execution -->
            <xsl:variable name="ActualParameters" select="$SubFlow/x1:ActualParameters|$SubFlow/xpdl:ActualParameters"/>
         ...
         <!-- TODO @join -->
         <xsl:variable name="TransitionRestrictions" select="x1:TransitionRestrictions|xpdl:TransitionRestrictions"/>
         <xsl:variable name="TransitionRestriction" select="$TransitionRestrictions/x1:TransitionRestriction|$TransitionRestrictions/xpdl:TransitionRestriction"/>
         <xsl:variable name="Join" select="$TransitionRestriction/x1:Join|$TransitionRestriction/xpdl:Join"/>
         <xsl:choose>
               <xsl:when test="$Join">
         ...
         <!-- TODO @performer -->
         <xsl:variable name="Performer" select="x1:Performer|xpdl:Performer"/>
         <xsl:if test="$Performer">
                <xsl:attribute name="performer"><xsl:value-of select="$Performer"/></xsl:attribute>
         </xsl:if>
         <xsl:variable name="QualityControlPerformer" select="x1:QualityControlPerformer|xpdl:QualityControlPerformer"/>
         ...
         <!-- TODO @split -->
         <xsl:variable name="Split" select="$TransitionRestriction/x1:Split|$TransitionRestriction/xpdl:Split"/>
         <xsl:choose>
               <xsl:when test="$Split">
         ...
         <xsl:choose>
            <xsl:when test="xpdl:Loop">
               <xsl:copy-of select="xpdl:Loop"/>
            </xsl:when>
            <xsl:otherwise>
               <!-- convert proprietary loop to standard xpdl loop -->
               <xsl:if test="($Activity/@LoopType = 'While') or ($Activity/@LoopType = 'Repeat')">
                  <xsl:element name="Loop" namespace="http://www.wfmc.org/2008/XPDL2.1">
                     <xsl:attribute name="LoopType">Standard</xsl:attribute>
                     <xsl:element name="LoopStandard" namespace="http://www.wfmc.org/2008/XPDL2.1">
                        <xsl:attribute name="TestTime">
                           <xsl:choose>
                              <xsl:when test="$Activity/@LoopType = 'While'">Before</xsl:when>
                              <xsl:otherwise>After</xsl:otherwise>
                           </xsl:choose>
                        </xsl:attribute>
                        <xsl:if test="$Activity/@LoopCondition">
                           <xsl:element name="LoopCondition" namespace="http://www.wfmc.org/2008/XPDL2.1"><xsl:value-of select="$Activity/@LoopCondition"/></xsl:element>
                        </xsl:if>
                     </xsl:element>
                  </xsl:element>
               </xsl:if>
   ...
   <xsl:template match="x1:Transition|xpdl:Transition">
      <transition>
         <!-- attributes -->
         <xsl:variable name="attrs" select="x1:ExtendedAttributes/x1:ExtendedAttribute|xpdl:ExtendedAttributes/xpdl:ExtendedAttribute"/>
         <xsl:variable name="CarnotExt" select="$attrs[@Name='CarnotExt']"/>
         ...
         <xsl:variable name="Condition" select="x1:Condition|xpdl:Condition"/>
         <xsl:variable name="Xpression" select="$Condition/x1:Xpression|$Condition/xpdl:Xpression"/>
         <xsl:choose>
   ...
   <xsl:template name="xpdl-description">
      <xsl:variable name="Description" select="x1:Description|xpdl:Description"/>
   ...
   <xsl:template name="carnot-attributes">
      <!-- Will get called with the carnot specific IExtensibleElement as current element, i.e. xpdl:ExtendedAttribute/c:Activity -->
