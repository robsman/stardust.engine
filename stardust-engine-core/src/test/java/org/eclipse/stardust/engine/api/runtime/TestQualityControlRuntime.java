...
public class TestQualityControlRuntime extends TestCase
{
   ...
   {
      adminServiceFactory = ServiceFactoryLocator.get("motu", "motu");
      as = adminServiceFactory.getAdministrationService();
      qs = adminServiceFactory.getQueryService();
      us = adminServiceFactory.getUserService();
      ws = adminServiceFactory.getWorkflowService();
      
      as.cleanupRuntimeAndModels(); 
      as.deployModel(getModelToDeploy(), DeploymentOptions.DEFAULT);
      ...
      qcManager.addGrant(QC_MANAGER_ID);
      qcManager.addGrant(ORGANIZATION_ID);
      ...
      monitoredUser.addGrant(MONITORED_USER_ID);
      monitoredUser.addGrant(ORGANIZATION_ID);
      monitoredUser = us.modifyUser(monitoredUser);
      
      regularUser = us.createUser("Dummy", "Dummy", "Dummy", "Dummy", "Dummy", null, null, null);
      regularUser.addGrant(MONITORED_USER_ID);
      regularUser.addGrant(ORGANIZATION_ID);
   ...
   public void testParticipantProbability()
   {
      ...
      QualityAssuranceAdminServiceFacade command 
         = new QualityAssuranceAdminServiceFacade(adminServiceFactory);
...
//   
   ...
   private List<DeploymentElement> getModelToDeploy()
   {
      List<DeploymentElement> models      
         = new ArrayList<DeploymentElement>(); 

      File file = new File(
            "C:/development/workspaces/runtime-trunk/sd/model/QAModel.xpdl");
      ...
      try
      {
         ...
         DeploymentElement model = new DeploymentElement(xmlString.toString().getBytes(XpdlUtils.ISO8859_1_ENCODING));
         models.add(model);
      }
      catch (Exception e)
      {
