...
public class CompleteEventActionTest
{
   public static final String COMPLETE_EVENTS_MODEL_NAME = "CompleteEventTestModel";
   ...
   public static final TestClassSetup testClassSetup = new TestClassSetup(
         ADMIN_USER_PWD_PAIR, ForkingServiceMode.NATIVE_THREADING, COMPLETE_EVENTS_MODEL_NAME);
   ...
   private int daemonExecutionInterval;
         ...
         throws Exception
   {
      ...
      try
      {
         findFirstCompletedActivityInstance();
         fail();
      }
      ...
      ActivityInstance ai = findFirstCompletedActivityInstance();
      assertNotNull(ai);
   }
    ...
    * <p>
    * See also <a
    * href="https://www.csa.sungard.com/jira/browse/CRNT-29147">CRNT-29147</a>.
    * </p>
    */
   @Test
   public void testEventActionReschedulingAfterLockingIssue() throws Exception
   {
      ...
      // lock the ai so the CompleteActivityEventAction will run into a
      ...
      // wait some time so the event processing starts and the CompleteActivityEventAction
      // will be executed (and fails with ConcurrencyException)
      Thread.sleep(daemonExecutionInterval * 2);

      // unlock the ai so CompleteActivityEventAction can succeed
      session.rollback();
      Thread.sleep(daemonExecutionInterval * 2);

      // before CRNT-29147: the concurrency action which is caught by
      // CompleteActivityEventAction
      // -> in the end the event action is only executed once and the process is NOT
      // completed
      // after CRNT-29147: the concurrency action is propagated to the event daemon,
      // the event action is retried, succeeds and the process IS completed
      ProcessInstance piCompl = findCompletedProcessInstance(pi);
      assertNotNull(piCompl);
   }

   private ProcessInstance findCompletedProcessInstance(ProcessInstance pi)
   {
      ProcessInstanceQuery query = ProcessInstanceQuery.findCompleted(pi.getProcessID());
      return queryService.findFirstProcessInstance(query);
   }

   private ActivityInstance findFirstCompletedActivityInstance()
   {
      final ActivityInstanceQuery aiQuery = ActivityInstanceQuery.findCompleted();
   ...
   private static final class AIStateMonitor implements IActivityInstanceMonitor
   {
      ...
      {
         if (((ActivityInstanceBean) activity).getState().equals(
               ActivityInstanceState.Suspended)
               && newState == ActivityInstanceState.APPLICATION)
         {
