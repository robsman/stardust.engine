 ...
 * flag enabled in conjunction with spawning subprocess functionality.
 * </p>
 *
 * @author Roland.Stamm
 */
@FixMethodOrder(MethodSorters.NAME_ASCENDING)
public class RootProcessAttachmentSpawnTest
{
   ...
   public void testUniquePerRootWithRootAttachment() throws Exception
   {
      ...
      ProcessInstance spawnSubprocessInstance = wfs.spawnSubprocessInstance(
            ai.getProcessInstanceOID(), "SpawnedSubProc", true, null);
      ActivityInstanceStateBarrier.instance()
            .awaitAlive(spawnSubprocessInstance.getOID());
      attachToProcess(getDoc(sf, spawnSubprocessInstance), sf,
            spawnSubprocessInstance.getOID());
      ...
      {
         Assert.assertEquals("2 documents expected", 2,
      ...
      Object inDataPathSubprocessInstance = wfs.getInDataPath(
            spawnSubprocessInstance.getOID(), "PROCESS_ATTACHMENTS");
      Assert.assertNotNull("PROCESS_ATTACHMENTS must be not null",
            inDataPathSubprocessInstance);
      if (inDataPath instanceof List)
      {
         Assert.assertEquals("2 documents expected", 2,
               ((List<Document>) inDataPathSubprocessInstance).size());
   ...
   public void testUniquePerRootWithoutRootAttachment() throws Exception
   {
      ...
      if (inDataPath instanceof List)
      {
         Assert.assertEquals("1 documents expected", 1,
               ((List<Document>) inDataPath).size());
         Document doc = ((List<Document>) inDataPath).get(0);
         Assert.assertNotNull("Doc should exist.", doc);
         Assert.assertEquals("spawnTest.txt should exist", "spawnTest.txt", doc.getName());
